...
import com.univ.it.ws.IWebServiceDataBase;
import com.univ.it.ws.WebServiceDataBaseService;
import com.univ.it.ws.WebServiceTable;
...
public class MainWindow extends Application {
    ...
    private IWebServiceDataBase currentDB;
    ...
    private boolean initRemoteDb() {
        try {
            WebServiceDataBaseService serviceImpl = new WebServiceDataBaseService();
            currentDB = serviceImpl.getWebServiceDataBasePort();
            return true;
        } catch (Exception e) {
            showErrorMessage("Web Service exception: " + e);
            e.printStackTrace();
            return false;
        }
    }
    ...
    private void createTable() {
        ...
        createNewTableButton.setOnAction(e -> {
            ...
            if (!tableNameTextField.getText().equals("")) {
                ...
                try {
                    currentDB.addTable(newTable.getName(), newTable.toString());
                } catch (Exception ex) {
    ...
    private void showDataBase() {
        List<WebServiceTable> tables;
        try{
            tables = currentDB.getTables().getItem();
        } catch (Exception e) {
            showErrorMessage(e.getMessage());
        ...
        for (WebServiceTable wsTable : tables) {
            Table table = null;
            try {
                table = new Table(
                        new BufferedReader(
                                new StringReader(wsTable.getTableStringRepresentation())
                        ),
                        wsTable.getTableName()
                );
            } catch (Exception e) {
                e.printStackTrace();
                showErrorMessage(e.getMessage());
    ...
    private void addNewRowTable() {
        ...
        try {
            ...
            WebServiceTable wsTable = currentDB.getTable(tableName);
            table = new Table(
                    new BufferedReader(new StringReader(wsTable.getTableStringRepresentation())),
                    tableName
            );
        } catch (Exception e) {
            showErrorMessage("No current table");
        ...
        addButton.setOnAction(e -> {
            ...
            try {
                currentDB.addRow(tableName, row.toString());
            } catch (Exception ex) {
                showErrorMessage(ex.toString());
            }
            newWindow.close();
    ...
    private void calculateDifference() {
        ...
        try {
            List<WebServiceTable> wsTables = currentDB.getTables().getItem();
            for (WebServiceTable wsTable : wsTables) {
                allTableNames.add(wsTable.getTableName());
            }
        } catch (Exception e) {
            showErrorMessage(e.getMessage());
            return;
        ...
        calculateDifference.setOnAction(e -> {
            ...
            try {
                currentDB.calculateDifferenceBetween(firstTableName, secondTableName);
            } catch (Exception e1) {
